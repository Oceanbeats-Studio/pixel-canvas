...
function updateMatches() {
  ...
  if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
    ...
    typedQuery = adjustColorCode(typedQuery, dir);

    if (isColorCode(typedQuery)) {
   ...
   ===================================== */
if (
  (focusMode === "hue" || focusMode === "code") &&
  (e.key === "ArrowRight" || e.key === "ArrowLeft")
) {
  // switch control back to HSL
  focusMode = "hue";
  

  huePos += e.key === "ArrowRight" ? 1 : -1;
  huePos = Math.max(0, Math.min(100, huePos));
  ...
  if (typedQuery.startsWith("#") || isColorCode(typedQuery)) {
    ...
    applyColorCodeAndSyncUI(typedQuery); // ✅ THIS LINE
  } else {
    focusMode = "name";
    matchIndex = -1;
  }
